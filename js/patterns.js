...
Pattern.prototype.steal = function (player) {
  ...
  for (var i = 0; i < 2 + 18 + 1; i++)
    savegame._u8array[this.offset + 0x2a + i] =
      savegame._u8array[player.offset + Offsets.PLAYER_ID1 + i];

  //change town id+name
  for (var i = 0; i < 2 + 18; i++)
    savegame._u8array[this.offset + 0x40 + i] =
      savegame._u8array[Offsets.TOWN_ID1 + i];

  this.author = new U16String(this.offset + 0x2c, 10).toString();
  this.canvas.title = this.title + ' by ' + this.author;
  el('pattern-preview-title').innerHTML =
    '<b>' + this.title + '</b> by ' + this.author;
...
function addPatternEvents(p) {
  addEvent(p.canvas, 'click', function () {
    showPatternDialog(p);
  });
}
