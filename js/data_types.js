...
NumericValue.prototype.set = function (newVal) {
  ...
  // Calculate Checksum
  var chk =
    ((enc >>> 0) + (enc >>> 8) + (enc >>> 16) + (enc >>> 24) + 0xba) &
    0xff &
    0xff;
  // Pack result
  this.int1 = enc;
  this.int2 =
    (adjust & 0xffff) + ((shift_val & 0xff) << 16) + ((chk & 0xff) << 24);
  this.value = this.decrypt();
  if (this.value !== newVal) {
    console.error('numeric value was not successfully encrypted/decrypted');
    MarcDialogs.alert(
      '<b>Unexpected error: </b>numeric value was not successfully encrypted/decrypted'
    );
  }
  this.changed = true;
};
...
BitArray.prototype._read = function () {
  for (var i = 0; i < this.length; i++)
    this.setBit(
      i,
      savegame.readU8(this.offset + parseInt(i / 8)) & (1 << i % 8)
    );
};
BitArray.prototype.save = function () {
  ...
  for (var i = 0; i < maxBytes; i++) {
    var b = 0;
    for (var j = 0; j < 8; j++) {
      if (this.ul.children[i * 8 + j].children[0].checked) b |= 1 << j % 8;
    }
...
BitArray.prototype.setAll = function (status) {
  for (var i = 0; i < this.length; i++) this.setBit(i, status);
};
...
PlayTime.prototype.getSeconds = function (v) {
  return this.time % 60;
};
PlayTime.prototype.getMinutes = function (v) {
  return parseInt(this.time / 60) % 60;
};
PlayTime.prototype.getHours = function (v) {
  return parseInt(this.time / (60 * 60)) % 24;
};
PlayTime.prototype.getDays = function (v) {
  return parseInt(this.time / (3600 * 24));
};
PlayTime.prototype.getTime = function () {
  return this.time;
};
PlayTime.prototype.setSeconds = function (s) {
  this.setTime(s, this.getMinutes(), this.getHours(), this.getDays());
};
PlayTime.prototype.setMinutes = function (m) {
  this.setTime(this.getSeconds(), m, this.getHours(), this.getDays());
};
PlayTime.prototype.setHours = function (h) {
  this.setTime(this.getSeconds(), this.getMinutes(), h, this.getDays());
};
PlayTime.prototype.setDays = function (d) {
  this.setTime(this.getSeconds(), this.getMinutes(), this.getHours(), d);
};
PlayTime.prototype.setTime = function (s, m, h, d) {
  this.time = (s % 60) + m * 60 + h * 3600 + d * 3600 * 24;
  this.toString();
};
...
PlayTime.prototype.save = function () {
  savegame.writeU32(this.offset, this.getTime());
};
...
U16String.prototype.set = function (s) {
  if (s.length > 0) {
    for (var i = 0; i < this.maxLength; i++) this.chars[i] = 0;
